// Generate random LOVE letters in background
const loveBg = document.querySelector('.love-bg');
const letters = ['L', 'O', 'V', 'E'];

for (let i = 0; i < 40; i++) {
  const span = document.createElement('span');
  span.innerText = letters[Math.floor(Math.random() * 4)];
...
nextBtn.addEventListener('click', () => {
  alert("NEXT STAGE: BOOK OPENING + EYES CARD");
});
nextBtn.addEventListener('click', () => {
